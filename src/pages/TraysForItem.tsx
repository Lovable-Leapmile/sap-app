...
const fetchTrays = async (itemId: string, inStation: boolean): Promise<Tray[]> => {
  const response = await fetch(`https://robotmanagerv1test.qikpod.com/nanostore/trays_for_order?in_station=${inStation}&item_id=${itemId}&order_type=outbound&like=false&num_records=10&offset=0&order_flow=fifo`, {
    headers: {
  ...
  return <div className="min-h-screen bg-background flex flex-col">
      ...
      <ScrollArea className="flex-1">
        <div className="container max-w-2xl mx-auto px-4 py-6 space-y-6">
          {/* Current Item Card */}
          {currentItem && <Card className="p-4 border-2 border-primary bg-primary/5">
          ...
          <div>
            ...
            <div className="space-y-3">
              ...
              {!stationError && stationTrays && stationTrays.length === 0 && <p className="text-center py-6 text-muted-foreground">No trays in station</p>}
              {stationTrays?.map(tray => {
              const trayOrder = trayOrders.get(tray.tray_id);
              return <Card key={tray.tray_id} className="p-4 border-2 border-primary/50 bg-primary/5">
                    <div className="space-y-3">
                      <div className="flex items-center justify-between">
                        ...
                        <div className="flex items-center gap-2">
                          {trayOrder && <span className="text-xs font-semibold px-2 py-1 rounded bg-blue-100 text-blue-700 dark:bg-blue-900 dark:text-blue-300">
                      ...
                      <div className="grid grid-cols-2 gap-2">
                        <Button onClick={() => handleRelease(tray)} disabled={isSubmitting || !trayOrder} variant="outline" className="w-full">
                          🔁 Release
                        </Button>
                        <Button onClick={() => handlePickItem(tray)} disabled={isSubmitting} className="w-full">
                          📦 Pick Item
                        </Button>
                      </div>
                    </div>
          ...
          <div>
            ...
            <div className="space-y-3">
              {storageError && !storageTrays && <p className="text-center py-6 text-destructive">Failed to load storage trays</p>}
              {!storageError && storageTrays && storageTrays.length === 0 && <p className="text-center py-6 text-muted-foreground">No trays in storage</p>}
              {storageTrays?.map(tray => <Card key={tray.tray_id} className="p-4 border-2 border-border">
                  <div className="space-y-3">
                    <div className="flex items-center justify-between">
                      <div className="flex items-center gap-2">
                        <Package className="text-primary" size={20} />
      ...
      <Dialog open={isPickingDialogOpen} onOpenChange={setIsPickingDialogOpen}>
        <DialogContent>
          ...
          <div className="flex items-center justify-center gap-4 py-6">
            ...
            <Button variant="outline" size="icon" onClick={() => setQuantityToPick(Math.min(selectedTray?.available_quantity || 1, quantityToPick + 1))} disabled={quantityToPick >= (selectedTray?.available_quantity || 1)}>
              <Plus size={20} />
            </Button>
          </div>
